...
**Code and documentation are in English; the product interface is in Spanish.**
- **All code and docs are in English**: comments, identifiers (variables, functions, types), object keys, commit messages, PR descriptions, documentation (README, `docs/**`), and **route paths/URLs** (e.g. `/nutrition`, `/habits`, `/goals`, `/day`, `/plans`).
